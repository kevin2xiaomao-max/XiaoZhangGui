...
struct V35HomeRevenueHero: View {
    ...
    var body: some View {
        Button(action: onTap) {
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("¥" + Fmt.groupedAmount(summary.revenue)).font(.system(size: 42, weight: .bold, design: .rounded)).monospacedDigit().lineLimit(1).minimumScaleFactor(0.62).foregroundStyle(V32.textOnHero).contentTransition(.numericText(value: summary.revenue))
            ...
            }.padding(20).frame(maxWidth: .infinity, alignment: .leading).background(LinearGradient(colors: [ThemeStore.shared.accentPalette.heroStart, ThemeStore.shared.accentPalette.heroEnd], startPoint: .topLeading, endPoint: .bottomTrailing), in: RoundedRectangle(cornerRadius: 28, style: .continuous))
